...
; est CORRECTE telle qu'ecrite: CH sauvegarde l'octet original
; et CL=4 sert uniquement de compte de decalage - pas de conflit.
; ------------------------------------------------------------
; Pour le montage, le signal RW du LCD est relié à la masse.
; ------------------------------------------------------------
; BUG TROUVE (pas dans SHR al,cl, mais dans le point d'entree de
; la boucle principale): le fichier precedent faisait
; ".ici  call lcd_init" suivi de ".halt: jmp .ici" - c'est-a-dire
; qu'apres avoir imprime "Hello World", le programme rebouclait
; pour RE-executer tout lcd_init (delai de mise sous tension
; ~440ms + sequence de reveil + un nouveau Clear Display) avant
; de reimprimer. Le texte n'etait donc visible que pendant ~1-2%
; de chaque cycle (~11ms visible sur ~480ms de cycle total) -
; largement trop bref pour l'oeil humain, d'ou "rien à l'écran"
; alors que le montage et le protocole LCD fonctionnaient. Cette
; version corrige ca en s'arretant reellement apres l'impression
; ("jmp .halt" au lieu de "jmp .ici").
; ------------------------------------------------------------
...
.ici:
        ...
        mov     cx, 1000        ; laisse le texte visible 1000 ms avant de
        call    delay_ms        ; reinitialiser/reimprimer (voir delay_ms plus bas)
...
; Calibre pour ~1ms a 4,77 MHz - approximatif, a ajuster si le
; rythme observe ne correspond pas a 1 seconde.
; ============================================================
