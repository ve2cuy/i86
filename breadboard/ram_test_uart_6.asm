...
; ------------------------------------------------------------
; ram_test_uart_6.asm
; nasm -f bin ram_test_uart_6.asm -o Z:\Partage\Alain\ram_test_uart_6.bin
; ------------------------------------------------------------
; DIAGNOSTIC suite a ton retour: "depuis la version 4, l'affichage
; UART pose probleme" (caracteres illisibles, mais qui se "reparent"
; toujours a la fin de chaque ligne - motif typique d'un BIT DE
; START PARASITE sur la ligne UART, pas d'une erreur de logique).
;
; Hypothese la plus probable: le port 10h (le latch UART/LED) et le
; port A du 8255 (LCD) sont sur le MEME bus de donnees. Si le latch
; du port 10h n'est pas qualifie par l'adresse (cad si son signal
; d'horloge/enable vient directement du WR/IOWC du CPU, SANS passer
; par un decodeur d'adresse), alors CHAQUE "out" vers le 8255 (LCD)
; reecrit AUSSI, par accident, le port 10h avec la valeur du LCD -
; ce qui inclut le bit0 = ligne UART TX! Un octet LCD dont le bit0
; est a 0 (frequent: 0x02, 0x28, la plupart des chiffres hexa...)
; ferait alors un front descendant parasite sur la ligne UART -
; exactement ce qui ressemble a un "faux bit de start" pour le
; recepteur, qui se desynchronise pendant quelques octets puis se
; resynchronise tout seul (d'ou "corrompu au debut, OK a la fin"
; de chaque ligne). Ca correspondrait aussi au fait que ca n'est
; apparu qu'a partir de la v4: c'est la PREMIERE fois que le code
; ecrit sur le 8255 (msg_bloc_progression) PENDANT que l'UART est
; utilise intensivement (avant, dans ram_test_uart_3.asm, le 8255
; n'etait ecrit qu'une fois au debut, avant le test RAM).
;
; A VERIFIER cote materiel: le signal d'enable/horloge du latch du
; port 10h passe-t-il par un decodeur qui verifie l'adresse (comme
; CS8255_ = NAND(A7,IO)), ou est-il directement branche sur WR/IOWC
; sans condition d'adresse ? Si c'est le cas, c'est la cause.
;
; En attendant, ce fichier ajoute un interrupteur logiciel pour
; ISOLER le probleme sans rien perdre du code: LCD_LIVE_UPDATE=0
; desactive les mises a jour de la ligne 2 du LCD PENDANT le test
; RAM (msg_bloc_progression) et le dump ROM (dump_line) - donc plus
; aucune ecriture au 8255 tant que l'UART travaille dans ces deux
; etapes. Si le texte UART redevient propre avec LCD_LIVE_UPDATE=0,
; ca confirme l'hypothese ci-dessus. Remettre a 1 une fois corrige
; (au niveau materiel) pour retrouver le suivi en direct sur le LCD.
; ------------------------------------------------------------
LCD_LIVE_UPDATE equ     0       ; 0 = diagnostic (voir ci-dessus), 1 = normal
STACK_SEG       equ     1000h
...
; prend environ 21 secondes. Ensuite, UNE ligne supplementaire
; est affichee pour les 16 DERNIERS octets de la ROM: c'est
; exactement le vecteur de reset (jmp C000h:0000h) suivi de la
; signature ' VE2CUY 26' - voir la fin du fichier.
;
; ATTENTION segment: le premier dump utilise ES=CS (C000h), qui ne
; peut adresser QUE les 64 Ko C0000h-CFFFFh (offset 16 bits max =
; FFFFh) - largement suffisant pour les 4 premiers Ko. Mais les 16
; DERNIERS octets de la ROM (256 Ko) sont a l'adresse physique
; FFFF0h-FFFFFh, hors de portee de CS:offset (3FFF0h ne tient pas
; sur 16 bits). On y accede avec le meme segment que le vecteur de
; reset materiel du 8088: F000h:FFF0h (F000h x 16 + FFF0h = FFFF0h)
; - c'est d'ailleurs l'adresse "canonique" que tout debogueur x86
; afficherait pour cette zone.
;
...
; (la meme qui part sur l'UART, en meme temps).
; ============================================================
...
.line_loop:
        ...
        ; (segment different - voir la note ci-dessus: C000h ne peut
        ; pas atteindre cette zone avec un offset 16 bits)
...
; complet: adresse/hexa/ascii, comme avant) et sur la ligne 2 du
; LCD (adresse seulement: "SSSS:OOOO", complete a 16 caracteres) -
; c'est le meme calcul d'adresse pour les deux, donc le LCD montre
; toujours exactement le bloc qui est en train de sortir sur l'UART.
;
...
%if LCD_LIVE_UPDATE
        ...
        ; pour que le LCD annonce le bloc au moment ou il part) - voir
        ; LCD_LIVE_UPDATE en tete de fichier: DESACTIVE par defaut ici,
        ; car c'est le suspect no.1 de la corruption UART signalee ---
        call    lcd_line2
        mov     ax, es
...
;        "SSSS:OOOO ERR   " (SSSS:OOOO = adresse de DEBUT du bloc,
;        meme valeur que le "debut" affiche sur l'UART)
...
%if LCD_LIVE_UPDATE
        ...
        ; sur 16 caracteres, sur la ligne 2 du LCD - DESACTIVE tant que
        ; LCD_LIVE_UPDATE=0 (voir l'en-tete du fichier: ecrire au 8255
        ; ICI, pendant que l'UART travaille, est le suspect no.1 de la
        ; corruption UART signalee depuis la v4) ---
        call    lcd_line2
...
;****************************************
;* proc1 - N'ecrit plus QUE sur le Port C
;* (le Port A est maintenant reserve au
;* LCD - voir lcd_hello_6.asm; l'ecrire
;* ici afficherait n'importe quoi au LCD)
;****************************************
proc1:
        OUT    PORTC,AL
	ret
...
; -------------------------------------------------------------------------------------------------
; Section suivante: LCD (HD44780, 4 bits, Port A du 8255) - voir
; lcd_hello_6.asm pour la version autonome et l'historique de mise
; au point (le protocole etait deja bon; c'est le passage du latch
; 10h au 8255 qui a fait fonctionner l'affichage).
; -------------------------------------------------------------------------------------------------
...
; descendant = capture reelle par le LCD).
...
; ============================================================
; lcd_line1 / lcd_line2
; Positionne le curseur au debut de la ligne 1 (DDRAM 00h) ou
; de la ligne 2 (DDRAM 40h, convention standard HD44780 2 lignes).
; ============================================================
lcd_line1:
        push    ax
...
; Positionne puis affiche une chaine (deja pad. a 16 car.).
; Entree: DS:SI = texte.
; ============================================================
lcd_show_line1:
        call    lcd_line1
...
; Equivalents LCD de uart_tx_hex_*: affichent une valeur en
; hexadecimal (majuscules) sur le LCD, a la position courante
; du curseur. Reutilisent hex_table. Detruisent AX/BX (jamais
; CX/DX/SI/DI/ES/BP - meme discipline que les versions UART).
; ============================================================
lcd_tx_hex_nibble:
        push    bx
        and     al, 0Fh
...
; ============================================================
; Delais LCD - PAS de lecture du drapeau "busy" possible dans ce
; montage (R/W force a la masse), donc tout est base sur des
; delais fixes, larges par prudence - memes valeurs que
; lcd_hello_6.asm, deja eprouvees sur le materiel.
; ============================================================
...
; ---- espaces via "times" - la largeur exacte de l'ecran evite
; ---- tout residu de l'etape/du bloc precedent affiche avant) ----
...
; ---- complement de 7 espaces utilise par dump_line, juste apres
; ---- les 9 caracteres d'adresse "SSSS:OOOO" (4+1+4) - 9+7 = 16 ----
lcd_txt_dump_pad:
                        times   7 db ' '
                        db      0

; ---- "queues" de 6 caracteres utilisees par msg_bloc_progression,
; ---- juste apres les 10 caracteres d'adresse deja affiches
; ---- (4+1+4+1) - 10+6 = 16 caracteres au total, exactement ----
lcd_txt_ok:             db      'OK'
