...
; ------------------------------------------------------------
; ram_test_uart_7.asm
; nasm -f bin ram_test_uart_7.asm -o Z:\Partage\Alain\ram_test_uart_7.bin
; ------------------------------------------------------------
; Reprend ram_test_uart_5.asm (dump ROM 4K+16 derniers octets, LCD
; en direct sur les 3 etapes) et corrige la cause reelle de la
; corruption UART trouvee et confirmee avec ram_test_uart_6.asm:
; le latch UART/LED n'etait qualifie par AUCUNE adresse (il
; reagissait a la ligne IO du 8088 seule, donc a TOUT "out"),
; ce qui le faisait recevoir par accident les octets destines au
; LCD/8255. Correction cote materiel: le latch est maintenant
; selectionne par NAND(A6,IO) - donc adresse changee de 10h a 40h
; (01000000b: A6=1, A7=0 - hors de la zone 80h-FFh du 8255, aucun
; chevauchement pour les adresses reellement utilisees). Le
; commutateur LCD_LIVE_UPDATE de la v6 n'est donc plus necessaire:
; le LCD peut de nouveau se mettre a jour en direct pendant le
; test RAM et le dump ROM, sans plus perturber l'UART.
;
; Aucun autre changement de logique: memes etapes 1/2/3, meme
; mapping de bits (bit0=TX) - seule l'ADRESSE du latch change.
;
