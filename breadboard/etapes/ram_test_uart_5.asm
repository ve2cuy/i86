...
; ------------------------------------------------------------
; ram_test_uart_5.asm
; nasm -f bin ram_test_uart_5.asm -o Z:\Partage\Alain\ram_test_uart_5.bin
; ------------------------------------------------------------
; Reprend ram_test_uart_4.asm (test RAM 128K + dump ROM + compte-
; rendu LCD en parallele sur le Port A du 8255) et modifie l'etape
; 3 (dump ROM):
;
;   - la ligne 2 du LCD suit maintenant la progression du dump:
;     a chaque ligne de 16 octets envoyee sur l'UART, l'adresse
;     REELLE de cette ligne (ES:DI, la meme qui part sur l'UART)
;     est affichee sur le LCD - voir dump_line plus bas;
;   - le dump est reduit aux 4 premiers Ko de la ROM (256 lignes,
;     au lieu de 16 Ko/1024 lignes) PLUS les 16 derniers octets
;     de la ROM (une ligne separee, adresse C000:3FF0h - c'est
;     exactement le vecteur de reset + la signature de fin de
;     ROM, voir la fin du fichier), pour verifier ces deux zones
;     sans attendre le dump complet.
;
; Les etapes 1 (test 8255) et 2 (test RAM) sont inchangees.
;
