...
test_ram:
        ...
        xor     dx, dx          ; DL = compteur de progression (paliers de 1024 octets)
                                 ; DH = segment du 1er octet defectueux (latche une seule
                                 ; fois, voir test_segment) - diagnostic temporaire
        xor     bp, bp          ; BP = offset du 1er octet defectueux (idem, diagnostic)

        ; --- Segment 0000h: physique 00000h-0FFFFh, teste en entier (64K) ---
        xor     ax, ax
        mov     es, ax
        xor     si, si          ; SI = 0 -> identifiant de segment (pour le diagnostic)
        xor     di, di
        ...
        mov     ax, STACK_SEG
        mov     es, ax
        mov     si, 1           ; SI = 1 -> identifiant de segment (pour le diagnostic)
        xor     di, di
        ...
        ; --- DIAGNOSTIC TEMPORAIRE: affiche l'adresse du 1er octet ---
        ; defectueux sur OUT 10h, en boucle, 3 valeurs successives:
        ;   1) DH  = segment (0 = physique 0xxxxh, 1 = physique 1xxxxh)
        ;   2) offset, octet de poids fort
        ;   3) offset, octet de poids faible
        ; Ne relance PAS de nouveau test (arret volontaire ici) pour
        ; laisser le temps de noter l'adresse exacte.
.show_fault:
        mov     al, dh          ; 1) segment du 1er defaut
        out     10h, al
        call    delai_blink
        call    delai_blink

        mov     ax, bp          ; BP n'est pas adressable en octets (pas de BPH/BPL)
        mov     al, ah          ; 2) offset, octet de poids fort
        out     10h, al
        call    delai_blink
        call    delai_blink

        mov     ax, bp
        out     10h, al         ; 3) offset, octet de poids faible
        call    delai_blink
        call    delai_blink

        jmp     .show_fault     ; repete l'affichage indefiniment
...
;          SI = identifiant de segment pour le diagnostic (0 ou 1,
;          voir test_ram) - non modifie, juste relu au 1er defaut
; Modifie: BH (leve a 1 si un octet est defectueux), DL (avance
;          d'un cran a chaque palier de 1024 octets testes),
;          BP/DH (latches l'adresse du 1er octet defectueux
;          uniquement - diagnostic temporaire, voir test_ram)
; ============================================================
test_segment:
.byte_loop:
        ...
        jmp     .restore
.fault:
        cmp     bh, 0           ; un defaut a-t-il deja ete detecte avant celui-ci ?
        jne     .flag_only      ; oui: ne pas ecraser l'adresse du 1er defaut deja latchee
        mov     bp, di          ; latche l'offset de CE premier octet defectueux
        mov     ax, si          ; SI n'est pas adressable en octets (pas de SIL/SIH en 16
        mov     dh, al          ; bits) - on passe par AX pour recuperer l'octet bas
.flag_only:
        mov     bh, 1           ; leve (ou confirme) le drapeau d'erreur global
.restore:
...
.blink_loop:
        ...
        out     10h, al
        call    delai_blink     ; delai long: chaque clignotement doit se voir
...
; ============================================================
; delai
; Delai COURT, utilise pour l'affichage de la progression du
; test memoire (tous les 1024 octets, voir test_segment) et par
; init_ram/read_ram. Doit rester court pour ne pas ralentir le
; test - ce n'est PAS ce delai qui rend les clignotements de fin
; de test visibles (voir delai_blink plus bas pour ca).
...
; ============================================================
; delai_blink
; Delai LONG, utilise uniquement par "blink" (clignotement de
; fin de test). Un delai separe de "delai" est necessaire ici:
; comme "delai" a ete raccourci pour ne pas ralentir le test de
; 128K, il est devenu trop rapide pour qu'un humain puisse voir
; et compter les clignotements de fin (3 = RAM valide, 5 = RAM
; defectueuse). "delai_blink" reprend la valeur d'origine (celle
; deja validee sur le montage pour l'alternance de LED du tout
; premier programme).
; ============================================================
delai_blink:
        push    bx
