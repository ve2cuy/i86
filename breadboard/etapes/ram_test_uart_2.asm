...
; ------------------------------------------------------------
; ram_test_uart_2.asm
; nasm -f bin ram_test_uart_2.asm -o Z:\Partage\Alain\ram_test_uart_2.bin
; ------------------------------------------------------------
...
;
; NOUVEAU dans cette version: apres le test de la RAM, dump
; complet de la ROM (256 Ko) au format classique adresse/hexa/
; ascii, ex: "0000:0000 FF FF FF ... FF : ................".
; Voir rom_dump plus bas pour le detail et l'estimation de duree.
;
...
.ici:
        ...
        call    rom_dump        ; dump complet de la ROM (256K) - voir plus bas
...
; ============================================================
; rom_dump
; Affiche le contenu complet de la ROM (256 Ko) au format:
...
;
; Adressage affiche: "logique", relatif au debut de la ROM
; (0000:0000 a 3FFF:FFF0), EXACTEMENT comme ce fichier source
; lui-meme (voir "ORG 0000h" en tete de fichier). Le segment
; materiel REEL correspondant (CS, = C000h sur ce montage) est
; ajoute au moment de la lecture (BP = CS, ES = BP + segment
; affiche) mais jamais affiche tel quel, pour rester coherent
; avec l'adressage du code source.
;
; ATTENTION - duree: 256 Ko / 16 = 16384 lignes d'environ 78
; caracteres chacune (~1.28 Mo de texte). A 9600 bauds
; (~960 car/s), ca prend environ 22 minutes. Ce n'est PAS un
; blocage: c'est la duree reelle du transfert serie.
; ============================================================
rom_dump:
        push    ax
        push    bx
        ...
        mov     ax, cs
        mov     bp, ax          ; BP = segment materiel reel de base (CS)

        xor     bx, bx          ; BX = segment "logique" affiche: 0000h,
                                 ; 1000h, 2000h, 3000h (4 x 64K = 256K)
.segment_loop:
        mov     ax, bx
        add     ax, bp
        mov     es, ax          ; ES = segment materiel reel correspondant
...
.print_char:
        ...
        cmp     di, 0           ; DI revenu a 0 -> les 65536 octets du
        je      .segment_done   ; segment courant sont entierement affiches
        jmp     .line_loop

.segment_done:
        add     bx, 1000h
        cmp     bx, 4000h       ; 4 segments de 64K = 256K -> termine
        jne     .segment_loop
...
txt_dump_banniere:      db      27,'[34m','=== Dump ROM 256 Ko (adressage logique 0000:0000-3FFF:FFF0) ===',27,'[0m',13,10
                        db      'Duree estimee a 9600 bauds: environ 22 minutes (~1.28 Mo de texte)',13,10,13,10,0
