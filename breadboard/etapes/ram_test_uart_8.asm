...
; ------------------------------------------------------------
; ram_test_uart_8.asm
; nasm -f bin ram_test_uart_8.asm -o Z:\Partage\Alain\ram_test_uart_8.bin
; ------------------------------------------------------------
; DIAGNOSTIC (suite au retour sur ram_test_uart_7.asm): avec le
; latch UART enfin correctement adresse (40h, NAND(A6,IO)), le
; dump ROM est PARFAIT sur l'UART - mais le test RAM montre encore
; exactement la meme corruption qu'avant (des blocs illisibles qui
; se "reparent" en fin de ligne). C'est une observation importante:
; le dump ROM fait aussi du LCD-en-direct (dump_line) intercale
; avec de l'UART, exactement comme msg_bloc_progression - si le
; dump est propre, la correction materielle du latch fonctionne
; bien pour le 8255/LCD en general. Donc la corruption qui persiste
; PENDANT le test RAM a forcement une cause DIFFERENTE de celle
; qu'on vient de corriger.
;
; Piste la plus probable: le test RAM (contrairement au dump, qui
; ne fait que LIRE la ROM) ecrit intensivement dans la RAM statique
; (10101010b/01010101b sur chaque octet, des dizaines de milliers
; de fois) - ce genre d'acces intensif peut generer du bruit
; electrique (appel de courant, rebond de masse) sur une breadboard,
; independamment du 8255/LCD. Pour le confirmer, ce fichier ajoute
; DEUX interrupteurs SEPARES (au lieu d'un seul comme dans la v6):
;
;   LCD_LIVE_UPDATE_DUMP = 1  -> dump_line continue de mettre a
;                                 jour le LCD en direct (deja prouve
;                                 propre, on n'y touche pas)
;   LCD_LIVE_UPDATE_RAM  = 0  -> msg_bloc_progression NE touche PLUS
;                                 au 8255/LCD du tout pendant le
;                                 test RAM (aucune ecriture 8255 nulle
;                                 part pres du test RAM)
;
; Si l'UART redevient propre pendant le test RAM avec ce reglage,
; ca confirme que ce n'est PLUS le 8255/LCD le coupable ici (deja
; exonere par le dump), et qu'il faut chercher du cote du bruit
; electrique genere par les ecritures RAM elles-memes (alimentation/
; masse/decouplage de la RAM). Si la corruption persiste MEME avec
; LCD_LIVE_UPDATE_RAM=0 (donc zero activite 8255 pendant tout le
; test RAM), ca confirme que le 8255/LCD n'y est pour rien: la
; cause serait alors ailleurs (bruit RAM, ou un probleme de timing
; UART marginal preexistant que le dump ne declenche simplement
; jamais).
; ------------------------------------------------------------
LCD_LIVE_UPDATE_DUMP    equ     1       ; deja prouve propre - laisser a 1
LCD_LIVE_UPDATE_RAM     equ     0       ; diagnostic - voir ci-dessus

STACK_SEG       equ     1000h

UART_PORT       equ     01000000b       ; 40h - latch UART/LED, maintenant
                                         ; selectionne par NAND(A6,IO)
                                         ; (avant: aucun decodage, port 10h)
...
start:
        ...
        mov     al, UART_IDLE
        out     UART_PORT, al         ; ligne UART au repos des le debut
...
.ok:
        ...
        call    uart_tx_string
.fin:
        ; --- NOUVEAU: meme information (adresse de debut + OK/ERR),
        ; condensee sur 16 caracteres, sur la ligne 2 du LCD ---
        ; DIAGNOSTIC: entierement desactive si LCD_LIVE_UPDATE_RAM=0 -
        ; aucune ecriture 8255 n'a alors lieu nulle part pendant le
        ; test RAM (voir l'en-tete du fichier).
%if LCD_LIVE_UPDATE_RAM
        call    lcd_line2
