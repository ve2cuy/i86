...
test_ram:
        ...
        xor     dx, dx          ; DL = compteur de progression; DH = segment du 1er defaut
        xor     bp, bp          ; BP = offset du 1er octet defectueux - DIAGNOSTIC (variante B)

        ; --- Segment 0000h: physique 00000h-0FFFFh, teste en entier (64K) ---
        xor     ax, ax
        mov     es, ax
        xor     si, si          ; SI = 0 -> identifiant de segment (diagnostic)
        xor     di, di
        ...
        mov     ax, STACK_SEG
        mov     es, ax
        mov     si, 1           ; SI = 1 -> identifiant de segment (diagnostic)
        xor     di, di
...
.fault:
        cmp     bh, 0
        jne     .flag_only
        mov     bp, di          ; DIAGNOSTIC (variante B)
        mov     ax, si
        mov     dh, al
.flag_only:
