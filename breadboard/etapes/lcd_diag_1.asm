...
start:
        mov     al, 00000000b   ; etat de repos DEFINI: E=0, RS=0, R/W=0, data=0000
        out     10h, al         ; fige le port AU PLUS TOT apres le reset - avant
                                 ; cette instruction, le latch peut etre dans un
                                 ; etat indetermine (E flottant/bruite), ce qui
                                 ; peut generer de faux fronts sur E pendant tout
                                 ; le delai de mise sous tension ci-dessous et
                                 ; corrompre les tout premiers quartets recus
                                 ; par le LCD (piste probable des caracteres
                                 ; aleatoires observes).
        cli
        mov     ax, STACK_SEG
        ...
        ; ------------------------------------------------------------
        ; DIAGNOSTIC MINIMAL: apres l'initialisation, on envoie UN SEUL
        ; caractere fixe ('A') a l'adresse DDRAM 0 (debut ligne 1), rien
        ; d'autre. Pas de boucle, pas de Clear supplementaire, pas de
        ; chaine, pas de changement d'adresse vers la ligne 2.
        ;
        ; But: isoler si le probleme vient du mecanisme de base
        ; (electrique/timing des quartets) ou de quelque chose de
        ; specifique a l'affichage d'une chaine complete / au
        ; changement d'adresse DDRAM.
        ;   - Si ce seul 'A' est deja incorrect/aleatoire -> le probleme
        ;     est dans lcd_init lui-meme ou dans le mecanisme electrique
        ;     de base (E/RS/data), pas dans lcd_print ou l'adressage.
        ;   - Si ce seul 'A' s'affiche correctement -> le probleme
        ;     apparait seulement avec des envois successifs (plusieurs
        ;     caracteres) ou avec le changement d'adresse ligne 2.
        ; ------------------------------------------------------------
        mov     al, 'A'
        call    lcd_data

.halt:
        jmp     .halt           ; on s'arrete ici, rien ne bouge apres
