...
; ------------------------------------------------------------
; ram_test_uart.asm
; nasm -f bin ram_test_uart.asm -o Z:\Partage\Alain\ram_test_uart.bin 
; ------------------------------------------------------------
...
;     SEGMENT:OFFSET en hexadecimal, colore en bleu, suivi d'un
;     "OK" (vert) ou "DEFAUT" (rouge);
...
;mov ecx, %1 ; Address of the string
...
test_ram:
        ...
        ret                     ; retourne a .ici (qui relance un nouveau cycle)
...
; appeler depuis test_segment au milieu du test).
; ============================================================
uart_tx_hex_nibble:
...
; "<bleu>SEG:debut-SEG:fin <blanc><vert>OK<blanc>"  (ou <rouge>DEFAUT)
...
; Bilan final du cycle de test (une ligne, en couleur).
; ============================================================
msg_ram_ok:
        mov     si, txt_ram_ok
        call    uart_tx_string
