...
; ------------------------------------------------------------
; ram_test_uart_4.asm
; nasm -f bin ram_test_uart_4.asm -o Z:\Partage\Alain\ram_test_uart_4.bin
; ------------------------------------------------------------
; Reprend ram_test_uart_3.asm (test RAM 128K + dump ROM 16 Ko,
; rapport UART 9600 8N1 - inchange) et y ajoute un compte-rendu
; en parallele sur le LCD (HD44780, mode 4 bits, Port A du 8255 -
; voir lcd_hello_6.asm), avec les 3 etapes du programme affichees
; sur la ligne 1:
;
;   1-Test 8255   -> ligne 2: pendant l'animation (effet1, qui
;                    n'utilise plus QUE le Port C maintenant que
;                    le Port A sert au LCD - voir proc1 plus bas)
;   2-Test RAM    -> ligne 2: mise a jour a CHAQUE bloc de 1 Ko
;                    teste, adresse de depart du bloc + OK/ERR,
;                    exactement en meme temps que le rapport UART
;   3-Dump ROM    -> ligne 2: plage d'adresses dumpee (fixe, le
;                    dump lui-meme reste uniquement sur l'UART)
;
...
;
; La ligne 2 du LCD n'est PAS mise a jour pendant le dump (elle
; garde la plage d'adresses affichee au debut de l'etape 3): le
; dump reste uniquement sur l'UART, pour ne pas ralentir la boucle.
; ============================================================
...
lcd_txt_step3_l1:       db      '3-Dump ROM'
                        ...
                        db      0
lcd_txt_step3_l2:       db      'C000:0000-3FF0'
